...
RANGE_NAME = 'KB_Update_' + datetime.datetime.now().strftime("%d-%m-%Y")
...
DAYS_TO_SKIP = 1
DAYS_TO_LOOKFORWARD = 1
DAYS_TO_LOOKBACK = 7

end_dt = datetime.datetime.now() - datetime.timedelta(days=DAYS_TO_SKIP)
...
bot_conv_queries = bot_conv_db.find_all_with_duration(start_dt, end_dt + datetime.timedelta(days=DAYS_TO_LOOKFORWARD))
...
if utils.is_sheet_present(SCOPES, SPREADSHEET_ID, RANGE_NAME, local_path):
    utils.delete_all_rows(SCOPES, SPREADSHEET_ID, RANGE_NAME, local_path)
else:
    utils.create_sheet(SCOPES, SPREADSHEET_ID, RANGE_NAME, local_path)
utils.add_rows(SCOPES, SPREADSHEET_ID, RANGE_NAME, questions_with_idks, local_path)
print(len(questions_with_idks))
